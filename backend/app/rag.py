...
async def retrieve_answer(db: Session, case_id: int, user_role: str, question: str) -> tuple[str, list[dict], list[dict], list[str]]:
    allowed_ids = get_allowed_chunk_ids(db, case_id, user_role)
    all_permissions = db.scalars(select(ChunkPermission).where(ChunkPermission.case_id == case_id)).all()
    ...
    result = get_chroma_collection().query(query_texts=[question], n_results=min(8, len(allowed_ids)), where={"$and": [{"case_id": case_id}, {"chunk_id": {"$in": allowed_ids}}]})

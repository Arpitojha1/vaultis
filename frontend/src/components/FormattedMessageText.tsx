...
export const FormattedMessageText: React.FC<FormattedMessageTextProps> = ({ text }) => {
  ...
  return (
    <div className="text-xs sm:text-sm leading-relaxed whitespace-pre-wrap font-sans space-y-1">
      {parts.map((part, index) => {
        ...
        if (part.startsWith('[FILTERED - NOT DISCLOSED') && part.endsWith(']')) {
          ...
          return (
            <span
              key={index}
              className="inline-flex flex-wrap items-center gap-1.5 px-2 py-0.5 mx-1 my-0.5 rounded-md bg-rose-100/90 border border-rose-300 text-rose-950 font-mono text-[11px] font-bold shadow-2xs select-none transition-all hover:bg-rose-200/90"
            ...
            >
              <span className="flex items-center space-x-1 text-rose-700">
                ...
                <span className="bg-rose-700 text-white text-[9px] font-extrabold px-1 py-0.2 rounded uppercase tracking-wider">
                  FILTERED - NOT DISCLOSED
                </span>
              </span>
              {description && (
                <span className="text-slate-900 font-sans font-semibold text-[11px] underline decoration-rose-300 decoration-1 underline-offset-2">
                  {description}
                </span>
              )}
            </span>

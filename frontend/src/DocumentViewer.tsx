...
export function DocumentViewer({ documentId, onClose }: { documentId: string; onClose?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-900/80 p-4 sm:p-8">
      <div className="mx-auto flex h-full w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-white dark:bg-slate-900 shadow-2xl transition-colors">
        <header className="flex items-center justify-between border-b dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-800 transition-colors">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-200">Document Viewer</h2>
          {onClose && (
            <button onClick={onClose} className="rounded-md p-1 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
        ...
        <div className="relative flex-1 bg-slate-100 dark:bg-slate-950 p-4 flex items-center justify-center transition-colors">
          ...
          {blobUrl && (
            <iframe
              src={blobUrl}
              className="h-full w-full rounded border dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm transition-colors"
